...
using CBinding
using Pkg.Artifacts
using FinancialStruct:HRetCode,HFileAccFlag,HFieldType,HFieldEncodeOp,HFieldFlag,HClientCreateFileOption
using FinancialStruct:type_tuple,ctype_tuple
using FinancialStruct:HDataField,HDataType,HDataItem,HCodeInfo
...
function generate_cstruct(data_types::Vector{HDataType}, len::Integer)::Tuple{Vararg{DataType}}
    ...
    for j = 1:len
        ...
        type_name = unsafe_string(pointer(reinterpret.(UInt8, [md_type.type...])))
        structdefine = string(structdefine,"typedef struct t_",type_name,"_HDB{\n")
        for i = 1:field_count
            field = fields[i]
            field_name = unsafe_string(pointer(reinterpret.(UInt8, [field.field_name...])))
...
function generate_cstruct(data_type::HDataType)::Tuple{Vararg{DataType}}
    ...
    type_name = unsafe_string(pointer(reinterpret.(UInt8, [data_type.type...])))
    structdefine = string(structdefine,"typedef struct t_",type_name,"_HDB{\n")
    for i = 1:field_count
        field = fields[i]
        field_name = unsafe_string(pointer(reinterpret.(UInt8, [field.field_name...])))
